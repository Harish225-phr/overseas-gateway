  ...
  return (
    <section className="py-20 md:py-28 relative overflow-hidden bg-primary">
      <div className="absolute inset-0 opacity-20">
        <div className="absolute top-0 left-0 w-96 h-96 rounded-full gradient-gold blur-3xl animate-blob" />
        <div className="absolute bottom-0 right-0 w-96 h-96 rounded-full bg-accent blur-3xl animate-blob" style={{ animationDelay: "5s" }} />
      </div>
      ...
      <div className="container relative z-10">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {reasons.map((r, i) => (
            <Reveal key={r.title} variant={i % 2 === 0 ? "left" : "right"} delay={i * 100}>
              <div className="card-shine group flex gap-4 p-6 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:border-accent hover:bg-white/10 hover:-translate-y-2 transition-bounce h-full">
                <div className="w-12 h-12 rounded-xl gradient-gold flex items-center justify-center shrink-0 group-hover:scale-110 group-hover:rotate-12 transition-bounce">
                  <r.icon className="w-6 h-6 text-accent-foreground" />
                </div>
                <div>
                  <h3 className="font-display font-semibold text-lg text-primary-foreground mb-1 group-hover:text-accent transition-smooth">
                    {r.title}
                  </h3>
